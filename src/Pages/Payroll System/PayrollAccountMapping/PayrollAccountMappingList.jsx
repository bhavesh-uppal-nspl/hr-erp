...
import {
  ...
  fetchPayrollAccountMapping,
  fetchPayrollEntries,
} from "../../../Apis/Payroll";

function PayrollAccountMappingList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchPayrollAccountMapping(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_account_mapping_id ,
            };
          });
          setBreak(b);
        })
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-account-mapping/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/account-mapping/edit/${item.id}`);
    },
  ...
  return (
    <>
      <Layout4
        loading={loading}
        heading={"Payroll Account Mapping"}
        btnName={"Add Mapping"}
        Data={Break}
      ...
      <TableDataGeneric
        tableName="Payroll Account Mapping"
        primaryKey="organization_payroll_account_mapping_id"
        heading="Payroll Account Mapping"
        data={Break}
...
export default PayrollAccountMappingList;
