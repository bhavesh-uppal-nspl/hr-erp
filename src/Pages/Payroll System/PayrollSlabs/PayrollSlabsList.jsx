...
import { fetchPayrollComponents, fetchPayrollComponentSlab } from "../../../Apis/Payroll";

function PayrollSlabsList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchPayrollComponentSlab(org?.organization_id)
        .then((data) => {
          let a = data?.payrollSlab;
          let b = a.map((item) => {
            return {
              ...item,
              id: item.organization_payroll_component_slab_id,
              payroll_component: item?.payroll_component?.payroll_component_name,
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-component-slab/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/component-slabs/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Payroll Component Slabs"
        primaryKey="organization_payroll_component_slab_id"
        heading="Payroll Component Slabs"
        data={Break}
...
export default PayrollSlabsList;
