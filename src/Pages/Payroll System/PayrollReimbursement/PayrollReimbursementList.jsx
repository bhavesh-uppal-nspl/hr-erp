...
import { fetchEmployeePayrollReimbursements, fetchPayrollComponents, fetchPayrollCycle } from "../../../Apis/Payroll";

function PayrollReimbursementList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchEmployeePayrollReimbursements(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          console.log("break", a);
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_reimbursement_id,
            };
          });
          setBreak(b);
        })
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-reimbursements/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/reimbursements/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Payroll Reimbursements"
        primaryKey="organization_payroll_reimbursement_type_id"
        heading="Payroll Reimbursements"
        data={Break}
...
export default PayrollReimbursementList;
