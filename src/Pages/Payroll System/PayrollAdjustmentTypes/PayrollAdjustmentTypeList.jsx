...
import { fetchEmployeePayrollAdjustment, fetchEmployeePayrollAdjustmentTypes } from "../../../Apis/Payroll";

function PayrollAdjustmentTypeList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchEmployeePayrollAdjustmentTypes(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          console.log("break", a);
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_adjustment_type_id,
            };
          });
          setBreak(b);
        })
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-adjustment-type/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/adjustment-types/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Payroll Adjustment Types"
        primaryKey="organization_payroll_cycle_id"
        heading="Payroll Adjustment Types"
        data={Break}
