...
import { fetchPayrollLoanTransaction, fetchPayrollPeriod } from "../../../Apis/Payroll";
...
function PayrollPeriodList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchPayrollPeriod(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          console.log("break", a);
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_period_id,
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-period/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/periods/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        ...
        primaryKey="organization_payroll_period_id "
        heading="Payroll Periods"
        data={Break}
        sortname={"period_name"}
        showActions={true}
        Route="/payroll/periods"
        DeleteFunc={deleteBreak}
        EditFunc={handleEdit}
