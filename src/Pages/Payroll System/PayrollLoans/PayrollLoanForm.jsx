...
import { fetchPayrollComponentTypes, fetchPayrollLoanTypes } from "../../../Apis/Payroll";
import { fetchOrganizationEmployee } from "../../../Apis/Employee-api";

function PayrollLoanForm({ mode }) {
  const { id } = useParams();
  ...
  const [loan, setLoanTypes]=useState([])
  ...
  useEffect(() => {
    ...
    useEffect(() => {
    {
      fetchPayrollLoanTypes(org?.organization_id)
        .then((data) => {
          setLoanTypes(data?.payroll);
        })
        .catch((err) => {
          setFormErrors(err.message);
        });
  ...
  useEffect(() => {
    let getdataById = async () => {
      const response = await axios.get(
        `${MAIN_URL}/api/organizations/${org.organization_id}/payroll-loan/${id}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "edit") {
        ...
        await axios.post(
          `${MAIN_URL}/api/organizations/${org.organization_id}/payroll-loan`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <Box px={4} py={4}>
      <Header
        mode={mode}
        updateMessage={"Payroll loans"}
        addMessage={"Payroll Loans"}
        homeLink={"/payroll/loans"}
        homeText={"Payroll Loans"}
      />
      {loading ? (
            ...
            <Paper elevation={4} sx={{ p: 3 }}>
              <Grid container spacing={2}>
                ...
                <TextField
                  ...
                  fullWidth
                  label="Loan Type"
                  name="organization_payroll_loan_type_id"
                  value={formData.organization_payroll_loan_type_id}
                  onChange={handleChange}
                  error={!!formErrors.organization_payroll_loan_type_id}
                  helperText={formErrors.organization_payroll_loan_type_id}
                  required
                >
                  {loan?.map((option) => (
                    <MenuItem
                      key={option.organization_payroll_loan_type_id}
                      value={option.organization_payroll_loan_type_id}
                    >
