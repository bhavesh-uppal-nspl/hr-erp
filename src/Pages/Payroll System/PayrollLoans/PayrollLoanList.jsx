...
import { fetchPayrollAdvances, fetchPayrollComponents, fetchPayrollLoan } from "../../../Apis/Payroll";

function PayrollLoanList() {
      ...
      fetchPayrollLoan(org?.organization_id)
        .then((data) => {
          ...
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_advance_id,
              loan_type:item?.payroll_loan_type?.loan_type,
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-loan/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/loans/edit/${item.id}`);
    },
  ...
  return (
    <>
      <Layout4
        loading={loading}
        heading={"Payroll Loans"}
        btnName={"Add Loan"}
        add_action={"PAYROLL_LOAN_ADD"}
        Data={Break}
      ...
      <TableDataGeneric
        tableName="Payroll Loans"
        primaryKey="organization_payroll_loan_id"
        heading="Payroll Loans"
        data={Break}
        sortname={"status"}
        showActions={true}
        Route="/payroll/loans"
        edit_delete_action={["PAYROLL_LOAN_DELETE", "PAYROLL_LOAN_EDIT"]}
        DeleteFunc={deleteBreak}
        EditFunc={handleEdit}
