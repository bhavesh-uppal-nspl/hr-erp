...
import { fetchPayrollPeriod } from "../../../Apis/Payroll";

function PayslipPaymentList() {
      ...
      fetchPayrollPeriod(org?.organization_id)
        .then((data) => {
          ...
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.employee_payslip_payment_id ,
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payslip-payment/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payslips/payments/edit/${item.id}`);
    },
...
export default PayslipPaymentList;
