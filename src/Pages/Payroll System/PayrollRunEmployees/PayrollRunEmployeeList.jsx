...
function PayrollRunEmployeeList() {
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-employee-runs/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/employee-runs/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Payroll Employee Runs"
        primaryKey="organization_payroll_run_employee_id"
        heading="Payroll Employee Runs"
        data={Break}
        sortname={"status"}
        showActions={true}
...
export default PayrollRunEmployeeList;
