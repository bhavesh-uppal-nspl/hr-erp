...
import { fetchPayrollAdvances, fetchPayrollComponents } from "../../../Apis/Payroll";

function AdvancePayrollList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchPayrollAdvances(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          console.log("break", a);
          let b = a.map((item) => {
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-advance/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/advance/edit/${item.id}`);
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Advance Payrolls"
        primaryKey="organization_payroll_advance_id"
        heading="Advance Payrolls"
        data={Break}
        sortname={"status"}
        showActions={true}
        Route="/payroll/advance"
...
export default AdvancePayrollList;
