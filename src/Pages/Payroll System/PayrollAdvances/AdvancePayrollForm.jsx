...
function AdvancePayrollForm({ mode }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    let getdataById = async () => {
      const response = await axios.get(
        `${MAIN_URL}/api/organizations/${org.organization_id}/payroll-advance/${id}`,
        {
          headers: {
  ...
  const validateForm = () => {
    ...
    if (!formData.advance_date)
      errors.advance_date =
        "Advance start date is required.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "edit") {
        ...
        await axios.post(
          `${MAIN_URL}/api/organizations/${org.organization_id}/payroll-advance`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      toast.success(
        mode === "edit"
          ? "Advance Payroll  updated!"
          : "Advance Payroll created!"
      );
      setFormErrors({});
      navigate(-1);
    } catch (err) {
  ...
  return (
    <Box px={4} py={4}>
      <Header
        mode={mode}
        updateMessage={"Advance Payroll"}
        addMessage={"Advance Payroll"}
        homeLink={"/payroll/advance"}
        homeText={"Advance Payroll"}
      />
      {loading ? (
            ...
            <Paper elevation={4} sx={{ p: 3 }}>
              <Grid container spacing={2}>
                ...
                <TextField
                  ...
                  error={!!formErrors.advance_amount}
                  helperText={formErrors.advance_amount}
                  required
                  inputProps={{
                    min: 0,
                    max: MAX_INCREMENT,
                ...
                <TextField
                  ...
                  onChange={handleChange}
                  error={!!formErrors.advance_date}
                  helperText={formErrors.advance_date}
                  required
                  InputLabelProps={{
                    shrink: true,
                  }}
...
export default AdvancePayrollForm;
