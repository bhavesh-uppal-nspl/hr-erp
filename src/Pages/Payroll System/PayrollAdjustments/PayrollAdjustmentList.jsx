...
function PayrollAdjustmentList() {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchEmployeePayrollAdjustment(org?.organization_id)
        .then((data) => {
          let a = data?.payroll;
          console.log("break", a);
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_payroll_adjustment_id,
            };
          });
          setBreak(b);
        })
  ...
  let deleteBreak = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/payroll-adjustment/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/payroll/adjustments/edit/${item.id}`);
    },
  ...
  return (
    <>
      <Layout4
        loading={loading}
        heading={"Payroll Adjustments"}
        btnName={"Add Adjustments"}
        Data={Break}
      ...
      <TableDataGeneric
        tableName="Payroll Adjustments"
        primaryKey="organization_payroll_adjustment_id"
        heading="Payroll Adjustments"
        data={Break}
        sortname={"adjustment_effect"}
        showActions={true}
        Route="/payroll/adjustments"
        DeleteFunc={deleteBreak}
