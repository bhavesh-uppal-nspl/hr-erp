...
const DEFAULT_COLUMNS = [
  ...
  },
  {
   ...
   {
    field: "work_shift_type",
    label: "work_shift_type",
    visible: true,
    width: 150,
    filterable: true,
    sortable: true,
      ...
      fetchWorkShift(org.organization_id)
        .then((data) => {
          ...
          let b = a.map((item) => {
           ...
           work_shift_type:item?.shift_type?.work_shift_type_name
...
const handleDelete = async (id) => {
  try {
    ...
    if (response.ok) {
      toast.success(data.message || "Workshift deleted successfully");
      setShifts((prev) => prev.filter((item) => item.id !== id));
      return;
    }

    // ❌ force an error to be caught
  ...
  const handleEdit = useCallback(
    ...
    (item) => {
      navigate(`/organization/work-shift/view/${item.id}`)
    },
    [navigate],
  )
  ...
  return (
    <>
      ...
      <TableDataGeneric
        ...
        edit_delete_action={["SHIFT_DELETE", "SHIFT_EDIT"]}
