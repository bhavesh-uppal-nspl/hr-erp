...
import { fetchOrganizationEmploymenentExiReasons } from "../../../Apis/Employment-exit-reason";
...
function OrganizationEmployementExitReasonsList() {
  ...
  useEffect(() => {
    if (org.organization_id) {
      setLoading(true);
      fetchOrganizationEmploymenentExiReasons(org?.organization_id)
        .then((data) => {
          ...
          let b = a.map((item) => {
            return {
              ...item,
              id: item.organization_employment_exit_reason_id,
            
              ExitReasonType:
                item?.exit_reason_type?.employment_exit_reason_type_name,
            };
          });
          setExitReasons(b);
        })
  ...
  let deleteexitreason = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/employemnt-exit-reason/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(
        `/organization-configration/employement-exit-reason/edit/${item.id}`
      );
    },
  ...
  return (
    <>
      ...
      <TableDataGeneric
        tableName="Exit Reason"
        primaryKey="organization_employment_exit_reason_id"
        heading="Exit Reason"
        data={ExitReasons}
...
export default OrganizationEmployementExitReasonsList;
