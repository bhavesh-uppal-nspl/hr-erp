                ...
                >
                  ...
                  >
                    ...
                    {DropDownData?.Employees?.filter(
                      ...
                      // Construct name from person data (first_name, middle_name, last_name)
                      // Check if name exists directly, otherwise construct from person fields
                      let employeeName = option?.name;
                      let employeeCode = option?.employee_code;

                      // If name doesn't exist, try to construct it from person data
                      if (!employeeName) {
                        const firstName = option?.first_name || option?.org_persons?.person?.first_name || "";
                        const middleName = option?.middle_name || option?.org_persons?.person?.middle_name || "";
                        const lastName = option?.last_name || option?.org_persons?.person?.last_name || "";
                      ...
                      // If employee_code doesn't exist, try to get it from org_persons.person_code
                      if (!employeeCode) {
                        employeeCode = option?.org_persons?.person_code || "";
                      }
                      ...
                      return (
                        <MenuItem
                          ...
                          value={option?.employee_id}
                        >
                          {employeeName ? `${employeeName} (${employeeCode || "N/A"})` : `Employee ID: ${option?.employee_id}`}
                        </MenuItem>
                      );
                    })}
                  </TextField>
