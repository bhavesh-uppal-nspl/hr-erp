...
import { fetchIntershipTypes } from "../../../Apis/InternManagement.js";

const DEFAULT_COLUMNS = [
  {
    field: "internship_type_name",
...
function IntershipTypesList() {
    ...
    const loadTableConfiguration = async () => {
      ...
      try {
        ...
        if (configRes.ok) {
          ...
          const orgKey = `Internship Types_grid_${org.organization_id}`;
  ...
  useEffect(() => {
    if (org?.organization_id) {
      ...
      fetchIntershipTypes(org?.organization_id)
        .then((data) => {
          ...
          let b = a.map((item) => {
            return {
              ...item,
              id: item?.organization_internship_type_id,
            };
          });
          setTypes(b);
        })
  ...
  let deleteStatus = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/internship-type/${id}`,
        {
          headers: {
  ...
  const handleEdit = useCallback(
    (item) => {
      navigate(`/intern/internship/types/edit/${item.id}`);
    },
    [navigate]
  );
...
export default IntershipTypesList;
