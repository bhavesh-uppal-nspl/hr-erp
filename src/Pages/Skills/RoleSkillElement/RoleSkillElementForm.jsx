...
import { fetchSkillElement, fetchSkills } from "../../../Apis/Skills";
import {
...
function RoleSkillElementForm({ mode }) {
  const { id } = useParams();
    ...
    let getdataById = async () => {
      ...
      if (a && !a.skills) {
        a = {
          ...
          skills: [
            {
              organization_skill_element_id : a.organization_skill_element_id ,
  ...
  const Type =["must_have" , "preferred" , "optional"]
  ...
  const validateForm = () => {
    ...
    formData.skills.forEach((skillItem, index) => {
      if (!skillItem.organization_skill_element_id )
        errors[`skills.${index}.organization_skill_element_id`] = "Skill element is required.";
      ...
      if (!skillItem.requirement_type)
        errors[`skills.${index}.requirement_type`] =
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === "edit") {
        ...
        await axios.post(
          `${MAIN_URL}/api/organizations/${org?.organization_id}/func-role-skill-element`,
          submitData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      toast.success(
        mode === "edit"
          ? "Role skills Element updated successfully!"
          : "Skills Element assigned to role successfully!"
      );
      setFormErrors({});
      navigate("/organization/func-role-skill-element");
    } catch (err) {
      console.error(err);
  ...
  return (
    <Box px={4} py={4} sx={{ backgroundColor: "#f5f7fa", minHeight: "100vh" }}>
      <Header
        mode={mode}
        updateMessage={"Functional Role Skill Element"}
        addMessage={"Functional Role Skill Element"}
        homeLink={"/organization/func-role-skill-element"}
        homeText={"Functional Role Skill Elements"}
      />
      {loading ? (
                ...
                >
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      Define required skill Elements with proficiency levels and
                      priorities
                    </Typography>
                  </Box>
                </Box>
                  ...
                  >
                    ...
                    <Grid container spacing={2}>
                        ...
                        value={
                          skill?.find(
                            (item) =>
                              ...
                              skillItem.organization_skill_element_id 
                          ) || null
                        }
                        onChange={(event, newValue) => {
                          handleSkillChange(
                            index,
                            "organization_skill_element_id",
                            newValue ? newValue.organization_skill_element_id  : ""
                          );
                        }}
                        renderInput={(params) => (
                          <TextField
                            ...
                            error={
                              !!formErrors[
                                `skills.${index}.organization_skill_element_id`
                              ]
                            }
                            helperText={
                              formErrors[
                      ...
                      >
                        ...
                        <Select
                          value={skillItem.requirement_type || ""}
                          onChange={(e) =>
                            handleSkillChange(
                              index,
                              "requirement_type",
...
export default RoleSkillElementForm;
