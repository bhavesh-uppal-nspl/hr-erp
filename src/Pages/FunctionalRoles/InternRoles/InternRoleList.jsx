...
import { fetchInternFunctionalRolesSpecial } from "../../../Apis/FunctionalManagment";
...
function InternRoleList() {
 ...
 function formatDate(date) {
  ...
  useEffect(() => {
    if (org?.organization_id) {
      setLoading(true);
      fetchInternFunctionalRolesSpecial(org?.organization_id)
        .then((data) => {
          ...
          let b = filteredData.map((item) => {
            return {
              ...item,
              id: item.intern_functional_role_id,
             intern_name: item?.intern
                ? `${item.intern?.first_name || ""} ${item?.intern?.middle_name|| ""} ${item?.intern?.last_name || ""}`
                : "",
  ...
  let deletedesignation = async (id) => {
    try {
      ...
      const response = await axios.delete(
        `${MAIN_URL}/api/organizations/${org_id}/intern-func-role/${id}`,
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        console.log("Intern Functional Roles deleted:", response.data.message);
      } else {
        const errorMessage =
  ...
  const handleEdit = useCallback(
    ...
    (item) => {
         ...
         navigate(`/intern/functional-role/edit/${item.id}`);
   ...
   const handleShow = useCallback(
    (item) => {
      navigate(`/intern/functional-role/view/${item.id}`)
    },
    [navigate],
  )
  ...
  return (
    <>
      <Layout4
        loading={loading}
        heading={"Intern Functional Roles"}
        btnName={"Add Roles"}
        Data={func}
        delete_action={"INTERN_FUNCTIONAL_ROLE_DELETE"}
        tableHeaders={[
        ...
        messages={[
          "Intern Functional Roles",
          "Intern Functional Roles",
          "Add Intern Functional Roless",
          "Intern Functional Roles",
        ]}
      ...
      <TableDataGeneric
        tableName="Intern Functional Roles"
        primaryKey="intern_functional_role_id"
        heading="Intern Functional Roles"
        data={func}
