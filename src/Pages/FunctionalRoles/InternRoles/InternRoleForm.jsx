 ...
 useEffect(() => {
  ...
  useEffect(() => {
    let getdataById = async () => {
      const response = await axios.get(
        `${MAIN_URL}/api/organizations/${org?.organization_id}/intern-func-role/${id}`,
        {
          headers: {
  ...
  const validateForm = () => {
    ...
    if (!formData.intern_id)
      errors.intern_id = "Intern is required.";
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "edit") {
        ...
        await axios.post(
          `${MAIN_URL}/api/organizations/${org?.organization_id}/intern-func-role`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      toast.success(
        mode === "edit"
          ? "Intern Functional Role updated!"
          : "Intern Functional Role created!"
      );
      setFormErrors({});
      navigate(-1);
    } catch (err) {
  ...
  return (
    <Box px={4} py={4}>
      <Header
        mode={mode}
        updateMessage={"Intern Functional Roles"}
        addMessage={"Intern Functional Role"}
        homeLink={"/intern/functional-role"}
        homeText={"Intern Functional Roles"}
      />
      {loading ? (
            ...
            <Paper elevation={4} sx={{ p: 3 }}>
              <Grid container spacing={2}>
                ...
                <Autocomplete
                  ...
                  value={
                    intern?.find(
                      (emp) => emp.intern_id === formData?.intern_id
                    ) || null
                  }
                  ...
                  disabled={mode === "view" || intern?.length === 0}
                  renderInput={(params) => (
                    <TextField
                      {...params}
